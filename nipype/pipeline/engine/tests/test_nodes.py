...
@pytest.mark.parametrize("inlist, index, expected",
                         (([[1, 2, 3], [4]], 1, [4]),
                          ([[1, 2, 3], [[4]]], 1, [[4]]),
                          ([[1, 2, 3], [[4, 5]]], 1, [[4, 5]]),
                          ([[1, 2, 3], [[4], [5]]], 1, [[4], [5]]),
                          ))
def test_outputmultipath_collapse(tmpdir, inlist, index, expected):
    """Test an OutputMultiPath whose initial value is ``[[x]]`` to ensure that
    it is returned as ``[x]``, regardless of how accessed."""
    select_if = niu.Select(inlist=inlist, index=index)
    select_nd = pe.Node(niu.Select(inlist=inlist, index=index),
                        base_dir=str(tmpdir),
                        name='select_nd')
    ...
    assert ifres.outputs.out == expected
    assert ndres.outputs.out == expected
    assert select_nd.result.outputs.out == expected
